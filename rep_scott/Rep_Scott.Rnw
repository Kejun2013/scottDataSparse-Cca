...
##comment out this artifical shift from scott
##comment out this artifical shift from scott
#personal shift
...
@
  ...
  \subsection{Add on Mar 9 2015}
  Average the genes with same neme.
<<Mar 9 2015>>=
dim(samples_Loessed)
samples_Loessed_average<-samples_Loessed[c(20,22,19,14,12,17,3,4,9,5,7,2)]
unique_genes_Loessed<-unique(samples_Annotation_OGS_Loessed)
for(i in unique_genes_Loessed){
  j<-which(samples_Annotation_OGS_Loessed==i)
  if(length(j)!=0){
    samples_Loessed_average<-rbind(samples_Loessed_average,
                                   colMeans(samples_Loessed_average[j,]))
  }
}
samples_Loessed_average<-samples_Loessed_average[-(1:length(samples_Annotation_OGS_Loessed)),]
rownames(samples_Loessed_average)<-unique_genes_Loessed
samples_Loessed_average<-t(samples_Loessed_average)
samples_Loessed_average<-samples_Loessed_average[,which(!is.na(colSums(samples_Loessed_average)))]
rownames(samples_Loessed_average)<-rownames(t(SEED2_number_reads))
@

  \subsection{Nov 9}
  no need for the following subsection anymore. 
  <<Nov.9,fig.align='center',fig.height=6,fig.width=7,echo=FALSE,results='markup',eval=FALSE>>=
...
@
 ...
 <<Nov 16,fig.align='center',fig.width=7,fig.height=6,echo=FALSE,results='hide',eval=FALSE>>=
library(PMA)
